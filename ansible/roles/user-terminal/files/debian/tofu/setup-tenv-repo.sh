...
function die {
    ...
    echo -e "$prefix: "
    echo -e "$prefix: ${bold}You can contact tofuutils for further assistance.${reset}"
    echo -e "$prefix: "
    
    echo -e "$prefix: ${bold}URL: https://github.com/tofuutils/tenv${reset}"
    echo -e "$prefix: "
...
function config_url {
    ...
    echo "https://dl.cloudsmith.io/public/tofuutils/tenv/config.deb.txt?$params" | sed 's/ /%20/g'
}
...
EOF
 ...
 >>>>: doesn't work after an hour, please contact tofuutils
 >>>>: for assistance.
 >>>>:
EOF
        }
...
function import_gpg_key {
    local text="Importing 'tofuutils/tenv' repository GPG keys ..."
    echo_running "$text"

    local gpg_keyring_path="/usr/share/keyrings/tofuutils-tenv-archive-keyring.gpg"
    
    curl -1sLf "https://dl.cloudsmith.io/public/tofuutils/tenv/gpg.8ACD4386ADD982F6.key" | gpg --dearmor >> $gpg_keyring_path
    ...
    echo_okfail "Checking for apt signed-by key support ..." || {
        mv ${gpg_keyring_path} /etc/apt/trusted.gpg.d/tofuutils-tenv.gpg
        chmod 644 /etc/apt/trusted.gpg.d/tofuutils-tenv.gpg
    }
...
function setup_repository {
    local repo_path="/etc/apt/sources.list.d/tofuutils-tenv.list"
    check_fetch_config

    local text="Installing 'tofuutils/tenv' repository via apt ..."
    echo_running "$text"
...
echo "Executing the  setup script for the 'tofuutils/tenv' repository ..."
echo
